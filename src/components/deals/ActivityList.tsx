...
import { DEAL_ACTIVITY_TYPE_LABELS, type DealActivity, type DealActivityType, type DealStage } from '@/types'
...
const activityIcons: Record<DealActivityType, (props: SVGProps<SVGSVGElement>) => ReactElement> = {
  note: (props) => (
  ...
  stage_change: (props) => (
    <svg {...baseIconProps(props)}>
      <path d="M7 7h10M7 7l3-3M7 7l3 3M17 17H7M17 17l-3 3m3-3-3-3" />
    </svg>
  ),
  proposal_sent: (props) => (
    <svg {...baseIconProps(props)}>
  ...
  if (activities.length === 0) {
    return (
      <EmptyState
        ...
        description="Registre ligações, e-mails e reuniões para manter o histórico deste negócio."
      />
    )
  }
    ...
    <>
      <ol className="relative border-l border-purple-200 pl-6">
        {activities.map((activity) => {
          ...
          return (
            <li key={activity.id} className="group mb-6 last:mb-0">
              ...
              <div
                className={`rounded-lg p-3 ${isStageChange ? 'bg-purple-50' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium uppercase tracking-wide text-purple-600">
                    {DEAL_ACTIVITY_TYPE_LABELS[activity.type]}
                  </span>
                  <div className="flex items-center gap-3">
                    <time className="text-xs text-neutral-400">{formatDateTime(activity.occurred_at)}</time>
                    {!isStageChange && (
                      <button
                        type="button"
                ...
                {stageMeta ? (
                  <p className="mt-1 flex items-center gap-2 text-sm text-neutral-700">
                    <span style={{ color: getStageConfig(stageMeta.from).color }}>
                      {getStageConfig(stageMeta.from).label}
                    </span>
                    <span className="text-neutral-400">→</span>
                    <span style={{ color: getStageConfig(stageMeta.to).color }}>
                      {getStageConfig(stageMeta.to).label}
                    </span>
                  </p>
                ) : (
