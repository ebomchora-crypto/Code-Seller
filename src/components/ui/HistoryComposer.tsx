...
export interface HistoryTypeMeta {
  label: string
  icon: LucideIcon
  color: string
}

interface HistoryComposerProps<T extends string> {
  types: T[]
  meta: Record<T, HistoryTypeMeta>
  placeholder: string
  emptyError: string
  onSubmit: (data: { type: T; content: string; occurred_at: string }) => Promise<void>
}
...
// Compositor de histórico (contatos e negócios): tipo em chips, texto e
// data, tudo num bloco só.
export function HistoryComposer<T extends string>({ types, meta, placeholder, emptyError, onSubmit }: HistoryComposerProps<T>) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSubmitting(true)
    await onSubmit({ type, content: content.trim(), occurred_at: new Date(occurredAt).toISOString() })
    setSubmitting(false)
    setContent('')
  ...
  return (
    ...
    >
      <div role="group" aria-label="Tipo de registro" className="flex flex-wrap gap-1.5 p-3 pb-0">
        {types.map((value) => {
          const { icon: Icon, color, label } = meta[value]
